...
def write_bundle(shipment: Shipment, out: pathlib.Path) -> None:
    ...
    canvas, fields = invoice.render(shipment)
    # optimize=False keeps PNG bytes a pure function of the pixels, which is
    # what determinism across machines rests on.
    canvas.image.save(out / "invoice.png", format="PNG", optimize=False, compress_level=6)
    (out / "invoice.json").write_text(
        json.dumps(
            {"document": "commercial_invoice", "fields": [f.as_dict() for f in fields]},
            indent=2,
            sort_keys=True,
        )
        + "\n"
...
def main() -> int:
    ...
    for n in range(args.count):
        ...
        print(f"{target}  {shipment.invoice_no}  {summary}")
    return 0
